...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
    ...
    $insertUserQuery = "INSERT INTO users (username, email, password, usertype, wnumber) VALUES ('$username', '$email', '$hashedPassword', '$usertype', '$wnumber')";
    ...
    if ($usertype == 'personal') 
    {
        ...
        $insertPersonalQuery = "INSERT INTO personallog (UserID, username, email, IDNo, IDproof, pFname, pLname, password, wnumber) VALUES ('$userID', '$username', '$email', '$IDNo', '$IDproofTargetPath', '$pFname', '$pLname', '$hashedPassword', '$wnumber')";
    ...
    } elseif ($usertype == 'showroom') 
    {
        ...
        $insertShowroomQuery = "INSERT INTO showroomlog (UserID, username, email, showname, location, licence, certificate, password, wnumber) VALUES ('$userID', '$username', '$email', '$showname', '$location', '$licenceTargetPath', '$certificateTargetPath', '$hashedPassword', '$wnumber')";
...
<head>
    ...
    #acceptTerms {
        margin-right: -250px; /* Adjust this value to set the space between the checkbox and the text */
        margin-left: -290px; /* Set margin-left to 0 to ensure checkbox is on the left */
    }
    ...
    #terms-link {
        ...
        margin-left: 0; /* Adjust this value to set the space between the checkbox and the text */
    }
...
<div class="container">
    ...
    <span id="terms-link" onclick="openTerms()">I accept the terms and conditions</span>
        </label>
...
<script>
    ...
    function validateForm() 
    {
        ...
        if (selectedType === 'personal') 
        {
            return validatePersonalFields() && validateTermsCheckbox();
        } 
        else if (selectedType === 'showroom') 
        {
            return validateShowroomFields() && validateTermsCheckbox();
        }
    ...
    function validateTermsCheckbox() 
    {
        ...
        if (!acceptTerms) 
        {
            alert('Please tick the checkbox to accept the terms and conditions.');
            return false;
        }
    ...
    function openTerms() {
        window.open('terms_and_conditions.php', '_blank');
    }
</script>
